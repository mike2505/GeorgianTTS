...
def setup_model(config, rank):
    ...
    return t3_fsdp, s3gen, ve, tokenizer, s3_tokenizer
...
def create_distributed_dataloaders(
    ...
    train_dataset = GeorgianTTSDataset(train_csv, tokenizer, s3_tokenizer, voice_encoder)
    val_dataset = GeorgianTTSDataset(val_csv, tokenizer, s3_tokenizer, voice_encoder)
...
def save_checkpoint(rank, model, optimizer, epoch, global_step, config):
    if rank != 0:
        return
...
def train_worker(rank, world_size, config_path, resume_from):
    ...
    t3_fsdp, s3gen, ve, tokenizer, s3_tokenizer = setup_model(config, rank)
    ...
    if rank == 0:
        print(f"Train batches: {len(train_loader)} per GPU")
    ...
    for epoch in range(start_epoch, config['training']['num_epochs']):
        train_loss, global_step = train_epoch(
            rank, t3_fsdp, train_loader, train_sampler, optimizer,
        ...
        if rank == 0:
            ...
            if (epoch + 1) % config['checkpointing']['save_every_n_epochs'] == 0:
                save_checkpoint(rank, t3_fsdp, optimizer, epoch, global_step, config)
...
def main():
    parser = argparse.ArgumentParser(description='FSDP training for Georgian TTS')
